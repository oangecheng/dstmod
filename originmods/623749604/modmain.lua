...
TECH = _G.TECH
TUNING = _G.TUNING
_G.STOREROOM_WORK_DIR = "workshop-623749604"
...
local function widgetcreation(widgetanimbank, widgetpos, slot_x, slot_y, posslot_x, posslot_y)
	...
	for y = slot_y, 0, -1 do
		for x = 0, slot_x do
			table.insert(storeroom.widget.slotpos, _G.Vector3(80*x-346*2+posslot_x, 80*y-100*2+posslot_y,0))
		end
	end

	containers.MAXITEMSLOTS  = math.max(containers.MAXITEMSLOTS, storeroom.widget.slotpos ~= nil and #storeroom.widget.slotpos or 0)
	
	containers.params.storeroom = storeroom
end

---------------------- position ----------------------
if GetModConfigData("Position") == ("Left") then
	widgetpos = _G.Vector3(-280,150,0)
elseif GetModConfigData("Position") == ("Center") then
	widgetpos = _G.Vector3(360-(STOREROOM.SLOTS*4.5),150,0)
elseif GetModConfigData("Position") == ("Right") then
	widgetpos = _G.Vector3(820-(STOREROOM.SLOTS*9.3),150,0)
end
...
AddPrefabPostInit("storeroom", function(inst)
	...
	inst.components.preserver:SetPerishRateMultiplier(TUNING.PERISH_STOREROOM_MULT)
	if TUNING.PERISH_STOREROOM_MULT < 1 then
		inst:AddTag("fridge")
	end
end)
