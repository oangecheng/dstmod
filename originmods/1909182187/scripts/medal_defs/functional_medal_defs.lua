	...
	if data.food ~= nil and data.food.components.edible ~= nil then
		...
		elseif data.food.components.perishable ~= nil then
			if data.food.components.perishable:IsFresh() then
				inst.components.talker:Say(GetString(inst, "ANNOUNCE_EAT", "GENERIC"))
...
medal_defs.wisdom_test_certificate={
	...
	onequipfn=function(inst,owner)
		--监听玩家读书
		inst.readbookfn = function(reader,data)
			local consume=5--默认消耗耐久
			--不是临时读者
			if not (data and data.istemporary) then
				--装备了新华字典,多扣3点耐久
				if data.hasdictionary then
					consume=consume+3
				else--否则少扣3点耐久
					consume=consume-3
				end
			...
			--消耗耐久
			if inst.components.finiteuses then
				consume=consume*TUNING_MEDAL.WISDOM_TEST.CONSUME_MULT
				inst.components.finiteuses:Use(consume)
				SpawnMedalTips(reader,consume,5)--弹幕提示
			end
		end
		owner:ListenForEvent("readbook", inst.readbookfn)--监听读书事件
	end,
	onunequipfn=function(inst,owner)
		owner:RemoveEventCallback("readbook", inst.readbookfn)
	end,
}
-------------------------------------------------智慧勋章-------------------------------------------------
medal_defs.wisdom_certificate={
	...
	onequipfn=function(inst,owner)
		...
		--监听读书
		inst.recoversanity=function(reader)
			--原本会读书的角色减少消耗
			if reader and not (reader.medal_t_com and reader.medal_t_com["reader"]) then
				reader.components.sanity:DoDelta(TUNING_MEDAL.WISDOM_MEDAL.SANITY_REDUCE)
			end
		end
		owner:ListenForEvent("readbook", inst.recoversanity)--监听读书事件
	...
	onunequipfn=function(inst,owner)
		...
		owner:RemoveEventCallback("readbook", inst.recoversanity)
		owner:PushEvent("refreshcrafting")--更新制作栏
	end,
}
-------------------------------------------------巧手考验勋章-------------------------------------------------
...
local function strokeFn(inst,player)
	...
	elseif player and player.components.talker and not player:HasTag("mime") then
		player.components.talker:Say(STRINGS.MEDAL_WORMWOOD_FLOWER_SPEECH.NOFERTILITY)
	end
end
		...
		if not inst:HasTag("blank_certificate") then
			...
			inst.picksomething = function(self,data)
				if data and data.loot then
					--采摘的是农场作物
					if data.object and string.sub(data.object.prefab,0,10)=="farm_plant" then
						local prefab=data.loot[1] and data.loot[1].prefab--获取采摘的预置物名
						--采摘的是巨型作物
						if prefab and string.sub(prefab,-10,-1)=="_oversized" then
...
local function finishedchop(inst,player,data)
	if data ~= nil and data.action ~= nil then
		if data.action.id and data.action.id=="CHOP" then
			...
			if data.target and data.target.components.growable then
				if data.target.prefab=="deciduoustree" and data.target.monster then
					consume=20--桦木精
				elseif data.target.prefab=="livingtree_halloween" then
					consume=6--万圣节活木树
				else
					consume=math.fmod(data.target.components.growable.stage,4)
				end
			elseif data.target and data.target.prefab=="livingtree" then
				consume=10--完全正常的树
			end
			--烧焦没经验
...
medal_defs.arrest_certificate={
	...
	onequipfn=function(inst,owner)
		...
		inst.justiceHit = function(self,data)
			...
			if data ~= nil and data.target and data.target.prefab=="krampus" and data.target.justiceplayer~=self then
				--直接砍死了，那就直接算击杀
...
medal_defs.justice_certificate={
				...
				if justice_targetlist[data.victim.prefab] then
					...
					if justice_value>=justice_targetlist[data.victim.prefab] and (data.victim.prefab~="klaus" or data.victim:IsUnchained()) then
						...
						elseif data.victim.prefab=="medal_rage_krampus" then
							data.victim.components.lootdropper:SpawnLootPrefab(math.random()<0.5 and "medal_treasure_map" or "klaus_sack")
						--打蝙蝠概率掉蓝图
		...
		inst.justiceHit = function(self,data)
			...
			if data ~= nil and data.target and (data.target:HasTag("monster") or justice_targetlist[data.target.prefab]) and data.target.justiceplayer~=self then
				...
				if justice_targetlist[data.target.prefab] and justice_value<justice_targetlist[data.target.prefab] then
					if self.components.talker and not self:HasTag("mime") then
						self.components.talker:Say(STRINGS.JUSTICEMEDALSPEECH.NOVALUE)
					end
					return
				end
	...
	extrafn=function(inst)--额外扩展函数
		inst.medal_honor={0,0,0,0}--荣誉:坎普斯数量,克劳斯数量,遗失包裹数量,怪物精华数量
	...
	onloadfn=function(inst,data)--扩展加载函数
		if data and data.medal_honor then
			...
			if not data.medal_honor[4] then
				inst.medal_honor[4]=0--怪物精华数量设为0(防止旧数据缺失导致数据读取异常)
			end
			--根据怪物精华数量变更耐久上限
			if inst.medal_honor[4]>0 then
				if inst.components.finiteuses then
	...
	extrafn=function(inst)--额外扩展函数
		...
		inst.components.writeable:SetOnWritingEndedFn(function(inst)
			if inst.components.medal_delivery then
				...
				inst.timepower=inst.timepower and inst.timepower+5 or 5--添加时间之力
			end
		end)
	end,
	onsavefn=function(inst,data)--扩展存储函数
		if inst.timepower then
			data.timepower=inst.timepower
		end
	end,
	onloadfn=function(inst,data)--扩展加载函数
		if data and data.timepower then
			inst.timepower=data.timepower
		end
	end,
}
-------------------------------------------------踏水勋章-------------------------------------------------
		...
		inst.onattackotherfn=function(self,data)
			...
			if data and data.weapon and inst.medal_level and inst.medal_level>0 then
				...
				if math.random() < tentacle_chance then
					...
					local offset = FindWalkableOffset(pt, math.random() * 2 * PI, 2, 3, false, true, function(pt) return not TheWorld.Map:IsPointNearHole(pt) end)
					if offset ~= nil then
	...
	onequipfn=function(inst,owner)
		...
		inst.valkyrieExamineKilled = function(self,data)
			...
			if data ~= nil and data.victim ~= nil and data.victim:HasTag("largecreature") and data.victim:HasTag("monster") then
				...
				--成功的喜悦
				if self.components.talker and not self:HasTag("mime") then
					self.components.talker:Say(STRINGS.VALKYRIETESTSPEECH.SUCCESS)
				end
			end
		end
		--给目标绑定处于女武神测验中的攻击者
		inst.valkyrieHit = function(self,data)
			local targetlist={"krampus","bat"}--目标列表
			--攻击目标符合条件，则进行相关处理
			if data ~= nil and data.target and data.target:HasTag("largecreature") and data.target:HasTag("monster") then
				...
				if self.components.talker and not self:HasTag("mime") then
					self.components.talker:Say(STRINGS.VALKYRIETESTSPEECH.REPEAT)
				end
			end
		end
		...
		inst.beDisqualified = function(self,data)
			if data and data.target then
				...
				if data.target:HasTag("smallcreature") then
					...
					--说话
					if self.components.talker and not self:HasTag("mime") then
						self.components.talker:Say(talkstring)
					end
				elseif data.target:HasTag("monster") then
					...
					if self.components.talker and not self:HasTag("mime") then
						self.components.talker:Say(STRINGS.VALKYRIETESTSPEECH.NOMONSTER)
					end
				end
			end
		end
...
medal_defs.valkyrie_certificate={
	...
	onequipfn=function(inst,owner)
		...
		inst.onmedalkilledfn=function(self,data)
			...
			if IsValidVictim(victim) then
				...
				if delta>0 then
					owner.components.health:DoDelta(delta, false, "medal_battleborn")
		...
		-- owner:ListenForEvent("mounted", inst.changecombatfn)--骑牛
		-- owner:ListenForEvent("dismounted", inst.changecombatfn)--停止骑
	...
	onunequipfn=function(inst,owner)
		...
		-- owner:RemoveEventCallback("mounted", inst.changecombatfn)
		-- owner:RemoveEventCallback("dismounted", inst.changecombatfn)
...
medal_defs.bee_king_certificate={
	...
	onequipfn=function(inst,owner)
		...
		-- owner:PushEvent("beeking_change")--更换蜂王身份(变更aoe伤害)
		owner:PushEvent("refreshcrafting")--更新制作栏
		...
		inst.onhitotherfn = function(self,data)
			...
			if not inst.isaoe then
				...
				if data.target and data.target.components.health and data.target.components.health:IsDead() then
					inst.poison_count=0
					inst.poison_target=nil
				elseif data.target then
					if inst.poison_target ~= data.target then
						inst.poison_count=0--目标和之前的不同，清空毒标记层数
						inst.poison_target=data.target--将目标标记为施毒目标
					end
					inst.poison_count=math.min(inst.poison_count + 1,TUNING_MEDAL.BEE_KING_MEDAL.POISON_MAX)--毒标记层数+1
					...
					inst:DoTaskInTime(0,function(inst)
						if data.target.components.health and not data.target.components.health:IsDead() then
							...
							data.target.components.health:DoDelta(-inst.poison_count*TUNING_MEDAL.BEE_KING_MEDAL.POISON_DAMAGE, nil, "bee_king_certificate",nil,nil,true)
						end
					end)
	...
	extrafn=function(inst)--额外扩展函数
		...
		if inst.components.rechargeable.SetOnChargedFn then
			inst.components.rechargeable:SetOnChargedFn(function(inst)
				--结束CD
				inst.poison_count=0--清空毒标记层数
				inst.poison_target=nil--移除绑定的目标
			end)
		end
	end,
}
...
medal_defs.blank_certificate={
	...
	--客户端扩展函数
	client_extrafn=function(inst)--额外扩展函数
		inst.grouptag="blankmedal"--默认勋章组
		--印刻勋章名网络变量
