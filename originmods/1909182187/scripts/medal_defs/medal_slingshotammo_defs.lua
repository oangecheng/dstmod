...
ammo_defs.medalslingshotammo_sandspike={
	...
	onhit = function(inst, attacker, target)
		...
		if pt and TheWorld.Map:GetTileAtPoint(pt.x, 0, pt.z) == GROUND.DESERT_DIRT then
			local sandspike = SpawnPrefab("sandspike")
...
ammo_defs.mandrakeberry={
	...
	onhit = function(inst, attacker, target)
		...
		if target.components.grogginess ~= nil then 
			target.components.grogginess:AddGrogginess(2, 6)
		elseif target.components.sleeper ~= nil then
			target.components.sleeper:AddSleepiness(5, 10)
		end
		--概率掉落种子(是直接吃的一半)
		if math.random()<TUNING_MEDAL.MANDRAKEBERRY_SEED_CHANCE*.5 then
