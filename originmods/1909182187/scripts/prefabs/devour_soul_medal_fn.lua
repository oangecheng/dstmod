...
--生成单个灵魂(坐标,受害者,是否直接算作掉落)
local function SpawnSoulAt(x, y, z, victim,drop)
    if drop then
...
--生成多个灵魂(受害者,灵魂数量,是否直接算作掉落)
local function SpawnSoulsAt(victim, numsouls, drop)
    local x, y, z = victim.Transform:GetWorldPosition()
    if numsouls == 2 then
        local theta = math.random() * 2 * PI--角度
        local radius = .4 + math.random() * .1--半径
		SpawnSoulAt(x + math.cos(theta) * radius, 0, z - math.sin(theta) * radius, victim,drop)
        theta = GetRandomWithVariance(theta + PI, PI / 15)
        SpawnSoulAt(x + math.cos(theta) * radius, 0, z - math.sin(theta) * radius, victim,drop)
    else
        SpawnSoulAt(x, y, z, victim,drop)
        if numsouls > 1 then
            numsouls = numsouls - 1
...
local function OnEntityDeath(inst, data)
    if data.inst ~= nil and data.inst.components.lootdropper == nil and inst:HasTag("soulstealer") then
