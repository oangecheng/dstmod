...
--掉落包裹(inst,是否为万圣节掉落,空白勋章数量)
local function DropBundle(inst,ishalloween,blank_medal_num)
	local bundleitems = {}
	if ishalloween then
		--万圣节糖果
		for i=1,4 do
...
local function exchangeGift(inst,player)
	...
	for k,v in ipairs(itemlist) do
		...
		if string.sub(v.prefab,1,8)~="trinket_" and v.prefab~="antliontrinket" and v.prefab~="blank_certificate" then
			if talker and not player:HasTag("mime") then
				talker:Say(STRINGS.EXCHANGEGIFT_SPEECH.FAIL2)
			end
			return false
		end
		--不能有堆叠的物品
		if v.components.stackable and v.components.stackable:IsStack() then
			if talker and not player:HasTag("mime") then
				talker:Say(STRINGS.EXCHANGEGIFT_SPEECH.FAIL1)
			end
			return false
		end
	...
	if #trinket_list<5 then
		if talker and not player:HasTag("mime") then
			talker:Say(STRINGS.EXCHANGEGIFT_SPEECH.FAIL3)
		end
		return false
	end
	--兑换成功
	if talker and not player:HasTag("mime") then
		talker:Say(halloween_toy_num>1 and STRINGS.EXCHANGEGIFT_SPEECH.HALLOWEEN or STRINGS.EXCHANGEGIFT_SPEECH.SUCCESS)
	end
	inst.components.container:DestroyContents()--销毁里面的物品
	inst.components.container:Close()--关闭容器
	DropBundle(inst,halloween_toy_num>1,blank_medal_num)--掉落包裹
	local fx = SpawnPrefab("collapse_small")
	fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
	fx:SetMaterial("wood")
	inst:Remove()
