...
local function bonus_damage_via_allergy(inst, target, damage, weapon)
	...
	--对中了蜂毒的目标造成额外伤害=(max(蜂毒层数-5,0)/10+向下取整(凋零值/20)*0.5+0.5)*蜂毒层数*额外伤害倍率
	if target.injured_damage then
		...
		extra_damage = extra_damage+(math.max(target.injured_damage-5,0)/10+math.floor(withered_num/20)*0.5+0.5)*target.injured_damage*TUNING_MEDAL.MEDAL_BEEGUARD_EXTRA_DAMAGE_MULT
	end
...
local function OnHitOther(inst,data)
	--炸毛状态记录总伤害
	if inst.puffy_damage then
		if data and data.damage then
			inst.puffy_damage=inst.puffy_damage+data.damage
		end
	end
end
...
local function OnDeath(inst)
	if inst and inst.puffy_damage then
		-- SpawnPrefab("lucy_transform_fx").Transform:SetPosition(inst.Transform:GetWorldPosition())--爆炸烟雾
		SpawnPrefab("medal_blood_honey_splat").Transform:SetPosition(inst.Transform:GetWorldPosition())
		local x, y, z = inst.Transform:GetWorldPosition()
		local ents=TheSim:FindEntities(x, y, z, 2 ,nil , {"INLIMBO", "NOCLICK", "catchable", "notdevourable","bee"},{"player","_combat"})
		local damage=inst.puffy_damage--爆炸伤害=自身吸血量
		if #ents>0 then
			for i,v in ipairs(ents) do
				if v.components.pinnable then
					v.components.pinnable:Stick("medal_blood_honey_goo")
				--造成自身吸血量的aoe伤害
				elseif v.components.combat and damage>0 then
					v.components.combat:GetAttacked(inst, damage, nil)
				end
			end
		end
		inst.puffy_damage=nil
	end
end
...
local function FocusTarget(inst, target)
    ...
    if target ~= nil then
        if inst.components.locomotor.walkspeed ~= TUNING_MEDAL.MEDAL_BEEGUARD_DASH_SPEED then
			...
			inst.puffy_damage=0--标记为炸毛状态,开始记录炸毛状态伤害
	...
	elseif inst.components.locomotor.walkspeed ~= TUNING_MEDAL.MEDAL_BEEGUARD_SPEED then
        if inst.puffy_damage then
			local queen = inst.components.entitytracker:GetEntity("queen")--获取实体跟踪器绑定的蜂王
			if queen and queen.components.health and not queen.components.health:IsDead() then
				local withered_num=queen.GetWithereNum and queen:GetWithereNum() or 0--获取凋零值
				local soldiers_num = queen.components.commander and queen.components.commander:GetNumSoldiers() or 0--获取守卫数量
				--吸血量=造成的伤害*max(凋零之蜂的凋零值,堕落之蜂数量)*0.5*难度系数
				local drink_blood=inst.puffy_damage*math.max(withered_num,soldiers_num)*0.5*TUNING_MEDAL.MEDAL_BEEGUARD_DRINK_BLOOD_MULT
				if drink_blood>0 then
					-- print("吸血:"..drink_blood)
					queen.components.health:DoDelta(drink_blood)--将吸来的血献祭给蜂王,献祭血量=吸血量*蜂王凋零值
					if queen.withered_num>0 then
						--降低凋零值，降低量=math.clamp(math.floor(吸血量/40),0.5,4)
						queen.withered_num=math.max(queen.withered_num-math.clamp(math.floor(drink_blood/40),0.5,4),0)
					end
				end
			end
			inst.puffy_damage=nil--取消炸毛状态标记
		end
