...
local function onequip (inst, owner)
    owner.AnimState:OverrideSymbol("swap_object", "swap_medal_fishingrod", "swap_fishingrod")
    owner.AnimState:OverrideSymbol("fishingline", "swap_medal_fishingrod", "fishingline")
    owner.AnimState:OverrideSymbol("FX_fishing", "swap_medal_fishingrod", "FX_fishing")
...
local pond_loot={
	...
	oasislake={--湖泊
		...
		--弯曲的叉子
		trinket_17 = { chance=0.1, consume=0.5, product="medal_losswetpouch7", },
	},
}
...
local function getLossPouch(inst,pond,fisher)
	...
	if lure then
		...
		if lure.components.oceanfishingtackle and lure.components.oceanfishingtackle.lure_setup ~= nil and lure.components.oceanfishingtackle.lure_setup.single_use then
			if pond.prefab=="medal_seapond" then
				--船上钓鱼池有食饵才有概率钓到时令鱼
