-- TODO use AddComponentPostInit instead
-- https://forums.kleientertainment.com/topic/70041-how-to-properly-save-this-overriden-component/
...
local OnExplosion = _ismastersim and function(src, data)
    if _state == QUAKESTATE.WAITING then
		if math.random() < 0.1 then
			SetNextQuake(_quakedata, GetTaskRemaining(_task) - data.damage*5)
		end
...
local iron_detected = KnownModIndex:IsModEnabled("workshop-1079879145") or KnownModIndex:IsModEnabled("workshop-737366433")
local tungsten_detected = KnownModIndex:IsModEnabled("workshop-1079879145") or KnownModIndex:IsModEnabled("workshop-358015908")

if iron_detected or tungsten_detected then

-- if "my moar metals" is enabled, earthquakes can spawn iron and tungsten

local metalized_percent = 0.03
local metalized_loot = {
            "marble"
        }
if iron_detected then
	table.insert(metalized_loot,"ore_iron")
	metalized_percent = metalized_percent+0.01
end
if tungsten_detected then
	table.insert(metalized_loot,"tungstenore")
	metalized_percent = metalized_percent+0.01
end


local metalized_debris = {
    { -- common
        weight = 0.88,
        loot = {
            "rocks",
    ...
    { -- rare
        weight = metalized_percent,
        loot = metalized_loot,
    },
}


self:SetDebris( metalized_debris )
...
self:SetQuakeData({
    ...
    debrispersecond = function() return math.random(4, 6) end,
    nextquake = function() return (TUNING.TOTAL_DAY_TIME*3 + math.random() * TUNING.TOTAL_DAY_TIME * 11) end,
    mammals = 1,
})
