        ...
        for _,data in pairs(recipes_data.Recipes) do
			...
			if not data.noatlas then
				if data.atlas then
					...
					atlas = "images/"..(data.product or data.name)..".xml"
				end
			end
			if not data.noimage then
				if data.image then
					...
					image = (data.product or data.name)..".tex"
				end
			end
            if not needHidden then
				...
				AddRecipe(data.name, data.ingredients[ingredientID], data.tab, level, data.placer, data.min_spacing, data.nounlock,data.numtogive, data.builder_tag, atlas, image, data.testfn ,data.product)
			end 
        end
    end
end
