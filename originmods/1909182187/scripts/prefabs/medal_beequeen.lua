...
local function bonus_damage_via_allergy(inst, target, damage, weapon)
	...
	--对中了蜂毒的目标造成额外伤害=(max(蜂毒层数-3,0)/5+向下取整(凋零值/10)+1)*蜂毒层数*额外伤害倍率
	if target.injured_damage then
		local withered_num=inst.GetWithereNum and inst:GetWithereNum() or 0
		extra_damage = extra_damage+(math.max(target.injured_damage-3,0)/5+math.floor(withered_num/10)+1)*target.injured_damage*TUNING_MEDAL.MEDAL_BEEQUEEN_EXTRA_DAMAGE_MULT
	end
...
local function OnAttacked(inst, data)
    if data.attacker ~= nil and not data.attacker:HasTag("shadowchesspiece") then
		...
		--如果是被玩家攻击了，去获取玩家的坐标，如果玩家没有脚踏实地，直接尖叫
		...
		if not screeched then
			--被特殊建筑攻击了，直接发出尖叫
			if data.attacker.prefab and table.contains(screech_loot,data.attacker.prefab) then
				inst:PushEvent("screech")
			end
		end
    end
...
local function scareFn(inst)
	local x,y,z = inst.Transform:GetWorldPosition()
	local taunt_must_tags={"_sanity","catapult","eyeturret","mosquito"}--需检测的标签
	--这里范围用投石机的范围
...
local function updateState(inst)
	...
	if inst.components.grouptargeter then
		...
		inst.spawnguards_threshold = math.max(TUNING_MEDAL.MEDAL_BEEQUEEN_TOTAL_GUARDS*player_num,TUNING_MEDAL.MEDAL_BEEQUEEN_TOTAL_GUARDS)--守卫数量阈值
	end
end
