...
local function Finished(inst, force_fx)
	...
	if inst.deploy_item_save_record ~= nil then
		...
		item:ListenForEvent("animover", item_foldup_finished)--折叠完毕
...
local function item_ondeploy(inst, pt, deployer)
	...
	--这里记录一下玩家的摄像头方向
	if deployer and deployer.player_classified and deployer.player_classified.net_medal_camera_heading then
		--通过网络变量向客机发起数据同步请求
		if deployer.player_classified.net_medal_camera_heading:value() then
			deployer.player_classified.net_medal_camera_heading:set_local(false)
			deployer.player_classified.net_medal_camera_heading:set(true)
		else
			deployer.player_classified.net_medal_camera_heading:set(true)
		end
		--网络变量需延迟获取
		obj:DoTaskInTime(0.4,function(inst)
