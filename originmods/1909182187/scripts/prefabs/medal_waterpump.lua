...
local function LaunchProjectile(inst)
	...
	--获取地皮信息函数
	if GetTileDataAtPoint==nil then
		if TheWorld and TheWorld.components.farming_manager and TheWorld.components.farming_manager.IsSoilMoistAtPoint then
			GetTileDataAtPoint=MedalGetLocalFn(TheWorld.components.farming_manager.IsSoilMoistAtPoint,"GetTileDataAtPoint")
		end
	end
    local x, y, z = inst.Transform:GetWorldPosition()
	...
	if #ents > 0 then
		...
		for _,v in ipairs(ents) do
			...
			--给田浇水
			elseif v:HasTag("needwater") and GetTileDataAtPoint then
				local data=GetTileDataAtPoint(false, pos.x, pos.y, pos.z)
				if data and data.soilmoisture and data.soilmoisture<MAX_SOIL_MOISTURE*.8 then
					-- print("湿度"..data.soilmoisture)
					targetpos=pos
					projectile_count=projectile_count+1
				end
			--给水壶灌水
...
local function OnStartChanneling(inst, channeler)
	...
	if inst.candrewwater then
		...
		inst.AnimState:Hide("fx")
		if channeler and channeler.components.talker and not channeler:HasTag("mime") then
			channeler.components.talker:Say(STRINGS.MEDALREMOULD_SPEECH.NEEDWATER)
		end
	end
end
--停止抽水
