...
local changeList={
	...
	--不朽果实12.5%
	immortal_fruit_oversized={medal_fruit_tree_immortal_fruit_scion=1,immortal_fruit=1,immortal_fruit_seed=6},
}

--变异函数
local function makeVariation(inst,target,doer)
	local newitem_name=nil
	if target.variation_prefab then
		newitem_name=target.variation_prefab--冥冥之中已有定数
...
local function fn()
	...
	inst.components.inventoryitem:ChangeImageName("halloweenpotion_moon")
