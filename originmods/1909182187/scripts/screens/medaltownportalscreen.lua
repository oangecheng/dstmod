...
function MedalTownportalScreen:LoadDests()
	local info_str = self.attach.deliverylist:value()--获取信息字符串
	local info_data=info_str~="" and json.decode(info_str)--目的地列表解包
...
function MedalTownportalScreen:DestListItem()
	...
	--移除坐标点按钮
	dest.deletebutton = dest:AddChild(
		TEMPLATES.StandardButton(
			--点击按钮执行的函数
			nil,
			-- STRINGS.DELIVERYSPEECH.CLOSEBUTTON,--按钮文字
		...
		--[[
		TEMPLATES.IconButton("images/button_icons.xml", "resurrect.tex", "移除坐标", false, false,
		        function()
		            print("移除坐标")
		        end,
		        {
	...
	dest.deletebutton:SetTextSize(18)
	dest.deletebutton:SetPosition(0, -30, 0)
	...
	dest.SetInfo = function(_, info)
        ...
        dest.consume:SetColour(1, 1, 1, 0.8)
		--传送按钮
...
function MedalTownportalScreen:RemovePos(index)
	...
	--传送组件
    local medal_delivery = self.attach.replica.medal_delivery
