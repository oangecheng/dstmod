...
TUNING_MEDAL ={
	...
	FISHING_MEDAL={--钓鱼勋章
		...
		MEDIUM_CONSUME_MULT = 0.7,--垂钓勋章钓到遗失塑料袋时消耗鱼饵的概率减免
		LARGE_CONSUME_MULT = 0.4,--渔翁勋章钓到遗失塑料袋时消耗鱼饵的概率减免
	},
	ARREST_MEDAL={--逮捕勋章
		MAXUSES = 10,--耐久
		CONSUME_MULT = gain_mult,--掉耐久的倍率
	...
	IMMORTAL_ESSENCE_ADDUSE = 20*gain_mult,--单次可补充耐久
	--流星法杖
	...
	------------------------------书籍---------------------------------
	--书籍作用范围
	BOOK_SACRIFICE_RADIUS = 5,--献祭范围(未解之谜,怪物图鉴)
	...
	JELLY_POWDER_CHARGE_MAX=2,--果冻粉单次最多可充电数量
	...
	--树根宝箱
	LIVINGROOT_CHEST_BIG_NEED = 14*condition_mult,--升到50格所需肥料包数8
	LIVINGROOT_CHEST_MID_NEED = 6*condition_mult,--升到25格所需肥料包数4
	LIVINGROOT_CHEST_SMALL_NEED = 2*condition_mult,--升到16格所需肥料包数2
	--船上钓鱼池
	...
	UPAPPETITE_HUNGER_CONSUME = -15,--吃开胃料理降饱食度
	--霸体
	...
	MEDAL_BUFF_AOECOMBAT_DIST = 2.5,--群伤范围
	MEDAL_BUFF_AOECOMBAT_MULT = 0.4,--群伤伤害继承比例
	MEDAL_BUFF_AOECOMBAT_LESS_MULT = 0.1,--群伤伤害继承比例(未佩戴蜂王勋章)
	--蜂毒(流血DEBUFF)
	...
	MEDAL_BEEQUEEN_TOTAL_GUARDS = 8,--守卫数量阈值
	MEDAL_BEEQUEEN_MIN_GUARDS_PER_SPAWN = 4,--单次最小生成守卫数量
	MEDAL_BEEQUEEN_MAX_GUARDS_PER_SPAWN = 5,--单次最大生成守卫数量
...
--取消部分特殊制作栏的特定标签，方便其他角色使用
if CUSTOM_RECIPETABS then
	--书籍
	if CUSTOM_RECIPETABS.BOOKS then
		CUSTOM_RECIPETABS.BOOKS.owner_tag=nil
		-- CUSTOM_RECIPETABS.BOOKS.crafting_station=true--不额外增加制作栏长度
	end
	--植物
	if CUSTOM_RECIPETABS.NATURE then
		CUSTOM_RECIPETABS.NATURE.owner_tag=nil
	end
	--工程
	if CUSTOM_RECIPETABS.ENGINEERING then
		CUSTOM_RECIPETABS.ENGINEERING.owner_tag=nil
	end
	--弹药
	if CUSTOM_RECIPETABS.SLINGSHOTAMMO then
		CUSTOM_RECIPETABS.SLINGSHOTAMMO.owner_tag=nil
	end
	--气球
	if CUSTOM_RECIPETABS.BALLOONOMANCY then
		CUSTOM_RECIPETABS.BALLOONOMANCY.owner_tag=nil
	end
	--蜘蛛
	if CUSTOM_RECIPETABS.SPIDERCRAFT then
		CUSTOM_RECIPETABS.SPIDERCRAFT.owner_tag=nil
	end
	--钟表
	--[[
	if CUSTOM_RECIPETABS.CLOCKMAKER then
		CUSTOM_RECIPETABS.CLOCKMAKER.owner_tag=nil
	end
	]]
end
