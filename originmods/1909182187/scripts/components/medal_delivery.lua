...
local Medal_Delivery =Class(function(self, inst)
	...
	self.getTownportals=function(inst,townportals)
		...
		if (townportals and #townportals>0) or (self.mark_list and #self.mark_list>0) then
			...
			local inst_text=inst
			for i, v in ipairs(townportals) do
				if v ~= inst then
					index_num=index_num+1
					local distance=v:GetDistanceSqToInst(inst)--距离
					local consume=math.floor(math.clamp(math.floor(math.sqrt(distance)/20), 10, 40)*TUNING_MEDAL.SPEED_MEDAL.CONSUME_MULT)--消耗
				...
				if medal.components.finiteuses then
					...
					if uses >= consume then
						...
						elseif pos then
							...
							--[[
							local success=Teleport(deliverier,nil,pos)--直接传送
							if success then
								medal.components.finiteuses:Use(self.consume)--扣除耐久
							elseif talker then
								talker:Say("那个地方我不能去")
...
function Medal_Delivery:OpenScreen(doer)
    if self.deliverier == nil then
        TheWorld:PushEvent("medal_townportalactivated", self.inst)--推送
		...
		local talker=self.inst and self.inst.components.talker--对话组件
		if talker then
			talker:Say(STRINGS.DELIVERYSPEECH.INUSE)--使用中
		end
	end
end
--关闭界面
...
function Medal_Delivery:AddMarkPos()
	...
	self.mark_list[#self.mark_list+1]={title=txt,x=x,y=0,z=z,worldid=TheShard:GetShardId()}
