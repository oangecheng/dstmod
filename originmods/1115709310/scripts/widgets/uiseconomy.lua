...
function uiseconomy:buildlist(title)
	...
	for i = 1+48*(self.numpage-1), math.min(#list, 48*(1+self.numpage-1)) do
		...
		local _atl = "images/inventoryimages.xml"
		if list[i].atlas then _atl = "images/inventoryimages"..list[i].atlas..".xml" end
		local _tex = list[i].name..".tex"
		if list[i].name == "onion" or list[i].name == "tomato" then _tex = "quagmire_"..list[i].name..".tex" end
		if list[i].name == "rock_avocado_fruit" then _tex = "rock_avocado_fruit_rockhard.tex" end
				...
				if down then
					...
					if self.down then
						...
						if control == CONTROL_ACCEPT then
							SendModRPCToServer(MOD_RPC["SimpleEconomy"]["sebuy"], list[i].name, false, Profile:GetLastUsedSkinForItem(list[i].name))
						elseif control == CONTROL_SECONDARY then
							SendModRPCToServer(MOD_RPC["SimpleEconomy"]["sebuy"], list[i].name, true, Profile:GetLastUsedSkinForItem(list[i].name))
						end
						self:StopUpdating()
					end
				end
